...
ChessMove AI::bestMove(const BoardPosition &position, uint8_t side, int32_t minMs, int32_t maxMs)
{
    cout << endl;
    ...
    tuple<ChessMove, int32_t> openingBookResult = this->openingBook.tryToFindMove(position);
...
tuple<int32_t, ChessMove> AI::BestMove(const BoardPosition &position, uint8_t side, int32_t depth, TranspositionTable &TransposTable)
{
...
tuple<int32_t, ChessMove> AI::alphaBetaMin(BoardPosition position, int32_t alpha, int32_t beta, int32_t depth_left, int32_t currentDepth, TranspositionTable &TransposTable)
{
    if (stopSearch)
        return make_tuple(0, ChessMove());
    if (currentDepth > maxDepth)
        maxDepth = currentDepth;

    if (depth_left == 0)
        return make_tuple(AI::alphaBetaMinOnlyCaptures(position, alpha, beta, currentDepth), ChessMove());
    ...
    {
        if (in_check)
            return make_tuple(AI::Infinity::Positive - currentDepth, ChessMove());
        return std::make_tuple(0, ChessMove());
    }
...
tuple<int32_t, ChessMove> AI::alphaBetaMax(BoardPosition position, int32_t alpha, int32_t beta, int32_t depth_left, int32_t currentDepth, TranspositionTable &TransposTable)
{
    if (stopSearch)
        return std::make_tuple(0, ChessMove());
    if (currentDepth > maxDepth)
    ...
    if (depth_left == 0)
        return std::make_tuple(AI::alphaBetaMaxOnlyCaptures(position, alpha, beta, currentDepth), ChessMove());
    ...
    {
        if (in_check)
            return std::make_tuple(AI::Infinity::Negative + currentDepth, ChessMove());
        return std::make_tuple(0, ChessMove());
    }
...
int32_t AI::alphaBetaMinOnlyCaptures(const BoardPosition &position, int32_t alpha, int32_t beta, int32_t currentDepth)
{
    if (stopSearch)
    {
        return 0;
...
int32_t AI::alphaBetaMaxOnlyCaptures(const BoardPosition &position, int32_t alpha, int32_t beta, int32_t currentDepth)
{
    if (stopSearch)
    {
        return 0;
