...
void deck::appendToBlackBD(const char* filename, std::string type_of_figure,sqlite3 *db)
{
    char* errMsg = nullptr;
    std::string value; 
    std::string index;
    ...
    std::string sql = "INSERT INTO BIGBLACKVADIMVLADYMTSEV (type, value, inndex) VALUES ('" + type_of_figure + "', '" + value + "', '" + index + "');";
    int rc = sqlite3_exec(db, sql.c_str(), nullptr, nullptr, &errMsg);
    char* err_msg = "BD is full";
    if (rc != SQLITE_OK) 
    {
...
int deck::size_of_BlackBD(sqlite3 *db)
{
    sqlite3_open("/home/danila/CCG/ProjectChess/src/DataBase/DB.db", &db); // открытие базы данных
    ...
    int size = sqlite3_exec(db, "SELECT COUNT(*) FROM BIGBLACKVADIMVLADYMTSEV", callback, &count, &errMsg);
...
void deck::blackShuffle(sqlite3 *db)
{
    const char* tableName = "BIGBLACKVADIMVLADYMTSEV";
...
std::string deck::get_random_BlackValue(sqlite3* db)
{
     // Получаем количество строк в таблице
    sqlite3_stmt* stmt;
    std::string sql = "SELECT COUNT(*) FROM BIGBLACKVADIMVLADYMTSEV;";
    ...
    // Получаем значение случайной строки начиная с 6-й строки
        ...
        do {
            ...
            sql = "SELECT * FROM BIGBLACKVADIMVLADYMTSEV LIMIT 1 OFFSET " + std::to_string(random_index) + ";";
            rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, NULL);
